...
export var Component = registerComponent('look-controls', {
  ...
  */
  ...
  updateOrientation: function () {
    ...
    // Check if physicalRotation is enabled
    if (this.data.physicalRotation) {
        const body = this.el.body; // Access the physics body
        if (!body) return;

        // Calculate rotation forces based on input
        const torque = new THREE.Vector3(
            -pitchObject.rotation.x * 10, // Adjust multiplier for sensitivity
            -yawObject.rotation.y * 10,
            0
        );

        console.log('Applying physical rotation torque:', torque);
